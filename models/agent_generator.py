...
from models.sequential_network import Seq_Network, Mu_Network, SNAF_Mu_Network
...
class AgentGenerator:
    ...
    def _generate_sac(self, model_config):
        lr = model_config.get('lr', 1e-3)
        gamma = model_config.get('gamma', 1)
        tau = model_config.get('tau', 1e-2)
        pi_model = Mu_Network([self.state_dim, 256, 128, self.action_dim], nn.ReLU(), nn.Tanh())
        q_model1 = Seq_Network([self.state_dim + self.action_dim, 256, 128, 1], nn.ReLU())
        q_model2 = Seq_Network([self.state_dim + self.action_dim, 256, 128, 1], nn.ReLU())
        
        noise = OUNoise(self.action_dim, threshold_decrease=(1/self.epoch_num))

        
        return SAC(self.action_min, self.action_max, q_model1, q_model2, pi_model,noise,
                    batch_size=self.batch_size, gamma=gamma, tau=tau, lr=lr)
 
    def _generate_snaf(self, model_config):
        lr = model_config.get('lr', 1e-3)
        gamma = model_config.get('gamma', 1)
        tau = model_config.get('tau', 1e-2)
        pi_model = SNAF_Mu_Network([self.state_dim, 256, 128, self.action_dim], nn.ReLU(), nn.Tanh())
        v_model1 = Seq_Network([self.state_dim, 256, 128, 1], nn.ReLU())
        v_model2 = Seq_Network([self.state_dim, 256, 128, 1], nn.ReLU())
        
        noise = OUNoise(self.action_dim, threshold_decrease=(1/self.epoch_num))

        
        return SNAF(self.action_min, self.action_max, v_model1, v_model2, pi_model,noise,
                    batch_size=self.batch_size, gamma=gamma, tau=tau, lr=lr)
    ...
    def generate(self, model_config):
        ...
        if model_name == 'naf':
            ...
            return self._generate_sac(model_config)
        elif model_name == 'snaf':
            return self._generate_snaf(model_config)
